...
{	
	...
	{
	  $sql = "select * from {$this->dbname} where {$where} {$order} limit {$limit}";
	  //echo ($sql);die;
	  $res = mysqli_query($this->link,$sql);
	  $data = array();
	...
	public function dbclassify($where=1)
	{
	  $sql = "select * from {$this->dbname} where {$where}";
	  //echo ($sql);die;
	  $res = mysqli_query($this->link,$sql);
	...
	//模糊查询
	public function dbgetlike($order,$putaway,$limit="0,5")
	{
	  //$sql = "select * from {$this->dbname} where `title` like '%{$order}%'  limit {$limit}";
	  $sql = "select * from {$this->dbname} WHERE `name` like '%{$order}%' and putaway='{$putaway}'  limit {$limit}";
	...
	{
		$sql = "select count(*) as count from {$this->dbname} where {$where}";
		//echo($sql);die;
		$res = mysqli_query($this->link,$sql);
		$row = mysqli_fetch_assoc($res);
		$conut = $row['count'];
	...
	public function scounts($order,$putaway)
	{
		$sql = "select count(*) as count from {$this->dbname} where `name` like '%{$order}%' and putaway='{$putaway}'  ";
		//echo($sql);die;
		$res = mysqli_query($this->link,$sql);
		$row = mysqli_fetch_assoc($res);
		$conut = $row['count'];
